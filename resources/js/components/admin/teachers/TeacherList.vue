...
const createUserSchema = yup.object({
    first_name: yup.string().label('First name').required().max(255),
    last_name: yup.string().label('Last name').required().max(255),
...
const createUser = (values, { resetForm, setErrors }) => {
    ...
    axios.post('http://127.0.0.1:8000/api/teachers', values)
    .then((response) => {
...
const updateUser = (values, {setErrors}) => {
    axios.put('http://127.0.0.1:8000/api/teachers/' + formValues.value.id, values)
        .then((response) => {
...
const deleteUser = () => {
    axios.delete(`http://127.0.0.1:8000/api/teachers/${userIdBeingDeleted.value}`)
    .then(() => {
