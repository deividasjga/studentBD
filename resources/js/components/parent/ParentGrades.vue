...
methods: {
    ...
    async fetchStudent() {
        try {
            ...
            await this.fetchStudentGrades(userId);
        } catch (error) {
    ...
    try {
        const response = await axios.get(`/api/getSelectedClass/${classId}`);
    ...
    try {
        const response = await axios.get(`/api/getSelectedStudentGrades/${userId}`);
